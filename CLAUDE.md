...
| **P4** | `SubScreen`          | Full-height bottom `Sheet` for one focused field. Own header: ✕ dismiss · title (+ optional subtitle). **Single-field editors commit live to the parent** — the ✕ just closes, no per-field Save (the parent's header Save is the real persist; header Cancel discards all). Only add a trailing **Save**/**Done** (`onSave`) when a sub-screen edits multiple fields that must commit as a unit. Nothing persists until the parent is saved. | `Sheet` (`side="bottom"`)             |
